...
class TestTable(unittest.TestCase):
    ...
    def test_create_sql(self):
        ...
        expected_sql = 'CREATE TABLE "users" ("id" INTEGER NOT NULL, "name" VARCHAR NOT NULL)'
        self.assertEqual(
            [expected_sql], self.platform.compile_create_sql(table)
        )
    ...
    def test_create_sql_with_unique_constraint(self):
        ...
        expected_sql = 'CREATE TABLE "users" ("id" INTEGER NOT NULL, "name" VARCHAR NOT NULL, UNIQUE(name))'
        self.platform.constraintize(table.get_added_constraints())
        self.assertEqual(
            self.platform.compile_create_sql(table), [expected_sql]
        )
    ...
    def test_create_sql_with_multiple_unique_constraints(self):
        ...
        expected_sql = 'CREATE TABLE "users" ("id" INTEGER NOT NULL, "email" VARCHAR NOT NULL, "name" VARCHAR NOT NULL, UNIQUE(name), UNIQUE(email))'
        self.platform.constraintize(table.get_added_constraints())
        self.assertEqual(
            self.platform.compile_create_sql(table), [expected_sql]
        )
    ...
    def test_create_sql_with_multiple_unique_constraint(self):
        ...
        expected_sql = 'CREATE TABLE "users" ("id" INTEGER NOT NULL, "email" VARCHAR NOT NULL, "name" VARCHAR NOT NULL, UNIQUE(name, email))'
        self.platform.constraintize(table.get_added_constraints())
        self.assertEqual(
            self.platform.compile_create_sql(table), [expected_sql]
        )
