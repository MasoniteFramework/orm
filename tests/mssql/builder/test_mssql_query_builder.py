...
class TestMSSQLQueryBuilder(unittest.TestCase):
    ...
    def test_where_like(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[age] LIKE '%name%'")

    def test_where_not_like(self):
        builder = self.get_builder()
        builder.where("age", "not like", "%name%")

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[age] NOT LIKE '%name%'")
    ...
    def test_select(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT [users].[name], [users].[email] FROM [users]")
    ...
    def test_create(self):
        ...
        self.assertEqual(builder.to_sql(), "INSERT INTO [users] ([users].[name], [users].[email]) VALUES ('Corentin All', 'corentin@yopmail.com')")

    def test_delete(self):
        builder = self.get_builder()
        builder.delete("name", "Joe", query=True)
        self.assertEqual(builder.to_sql(), "DELETE FROM [users] WHERE [users].[name] = 'Joe'")

    def test_where(self):
        builder = self.get_builder()
        builder.where("name", "Joe")

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[name] = 'Joe'")
    ...
    def test_offset(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] OFFSET 5 ROWS FETCH NEXT 1 ROWS ONLY")

    def test_join(self):
        builder = self.get_builder()
        builder.join("profiles", "users.id", "=", "profiles.user_id")

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] INNER JOIN [profiles] ON [users].[id] = [profiles].[user_id]")

    def test_left_join(self):
        builder = self.get_builder()
        builder.left_join("profiles", "users.id", "=", "profiles.user_id")
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] LEFT JOIN [profiles] ON [users].[id] = [profiles].[user_id]")


    def test_right_join(self):
        builder = self.get_builder()
        builder.right_join("profiles", "users.id", "=", "profiles.user_id")
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] RIGHT JOIN [profiles] ON [users].[id] = [profiles].[user_id]")

    def test_update(self):
        builder = self.get_builder().update(
            {"name": "Joe", "email": "joe@yopmail.com"}, dry=True
        )
        self.assertEqual(builder.to_sql(), "UPDATE [users] SET [users].[name] = 'Joe'[users].[email] = 'joe@yopmail.com'")

    def test_increment(self):
        builder = self.get_builder()
        builder.increment("age", 1)
        self.assertEqual(builder.to_sql(), "UPDATE [users] SET [users].[age] = [users].[age] + '1'")
    ...
    def test_order_by_asc(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] ORDER BY [users].[email] ASC")

    def test_order_by_desc(self):
        builder = self.get_builder()
        builder.order_by("email", "desc")
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] ORDER BY [users].[email] DESC")

    def test_where_column(self):
        builder = self.get_builder()
        builder.where_column("name", "username")
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[name] = [users].[username]")

    def test_where_not_in(self):
        builder = self.get_builder()
        builder.where_not_in("id", [1, 2, 3])
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[id] NOT IN ('1','2','3')")

    def test_between(self):
        builder = self.get_builder()
        builder.between("id", 2, 5)
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[id] BETWEEN '2' AND '5'")

    def test_not_between(self):
        builder = self.get_builder()
        builder.not_between("id", 2, 5)
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[id] NOT BETWEEN '2' AND '5'")

    def test_where_in(self):
        builder = self.get_builder()
        builder.where_in("id", [1, 2, 3])

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[id] IN ('1','2','3')")

    def test_where_null(self):
        builder = self.get_builder()
        builder.where_null("name")

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[name] IS NULL")

    def test_where_not_null(self):
        builder = self.get_builder()
        builder.where_not_null("name")

        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[name] IS NOT NULL")
    ...
    def test_having(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT [payments].[user_id], AVG([payments].[salary]) AS salary FROM [payments] GROUP BY [payments].[user_id] HAVING [payments].[salary] >= '1000'")

    def test_group_by(self):
        builder = self.get_builder(table="payments")
        builder.select("user_id").min("salary").group_by("user_id")

        self.assertEqual(builder.to_sql(), "SELECT [payments].[user_id], MIN([payments].[salary]) AS salary FROM [payments] GROUP BY [payments].[user_id]")
    ...
    def test_where_lt(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[age] < '20'")
    ...
    def test_or_where(self):
        ...
        self.assertEqual(builder.to_sql(), "SELECT * FROM [users] WHERE [users].[age] = '20' OR [users].[age] < '20'")
